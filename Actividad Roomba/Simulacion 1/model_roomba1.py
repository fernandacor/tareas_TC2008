...
class RoombaModel(Model):
    ...
    """
    def __init__(self, N, width, height, obstacles, dirtycells):
        self.num_agents = N
        ...
        # pos_gen = lambda w, h: (self.random.randrange(w), self.random.randrange(h)) # esto hace que sea random
        pos_gen = lambda w, h: (1, 1)

        # Add the agent to a random empty grid cell
        for j in range(self.num_agents):

            a = RoombaAgent(j+2000, self) 
            self.schedule.add(a)
